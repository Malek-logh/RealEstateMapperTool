...
def scrape_page(url):
    ...
    results = (title, city, property, area , price, longitude, latitude, url)
    return results

# Function to generate the URL for a specific page number
def get_url(NbPage):
    template = 'https://www.mubawab.tn/fr/sc/terrains-a-vendre:p:{}'
...
def main(Nbpage):
    ...
    df = pd.DataFrame(data, columns=['title', 'city', 'property', 'area(m²)', 'price(TND)', 'longitude', 'latitude', 'link'])
    ...
    df.to_csv('MubawabTerrain.csv', index=False)

# Running the main function with 106 pages to scrape
main(106)
